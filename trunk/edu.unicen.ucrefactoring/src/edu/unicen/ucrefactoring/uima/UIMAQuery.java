package edu.unicen.ucrefactoring.uima;
...
public class UIMAQuery {
	...
	public UIMAQuery(EList<EObject> uimaRoots) {		
	...
	public EList<DomainNumber> getDomainNumbers(Section section) {
		SELECT statement = new SELECT(new FROM(uimaRoots), new WHERE(cDomainNumber.AND(cRange(section))));
		IQueryResult result = statement.execute();
		return fromIQueryResultToEList(result, new BasicEList<DomainNumber>());
	}
